...
# ------------------------------------------------------------------------
# PARAMÈTRES GLOBAUX (utilisés par la classe)
# ------------------------------------------------------------------------
...
# Exemple de coûts (FCFA/m) par diamètre (vous pouvez compléter selon votre table réelle)
COUT_DIAMETRE = {
    40 :  5000,
    63 :  7000,
    75 :  8500,
...
class OptimisationReseau:
    ...
    def initialiser_population(self):
        ...
        parmi les diamètres disponibles dans COUT_DIAMETRE."""
        population = []
    ...
    def evaluer_solution(self, solution):
        """
        Score = coût total + pénalités (pressions et vitesses hors bornes).
        """
        try:
            ...
            score = cout_total + penalites
            return (score,)

        except Exception as e:
            print(f"Erreur evaluer_solution : {e}")
            return (float('inf'),)
    ...
    def evaluer_solution_multi(self, solution):
        """
        Retourne deux objectifs à minimiser :
        (coût_total, énergie).
        """
        try:
            # Appliquer diamètres
            ...
            # On ajoute des pénalités pour pressions/vitesses hors bornes
            penalites = self.calculer_penalites(resultats)
            ...
            print(f"Erreur evaluer_solution_multi : {e}")
    ...
    def calculer_penalites(self, resultats):
        ...
        Rappel : si pression EPANET est en mCE, PRESSION_MIN et MAX sont en mCE aussi.
        """
        penalites = 0
    ...
    def calculer_energie(self, resultats):
        """
        Ex. Somme(rho*g*Q*Headloss) sur le temps => kWh
        """
        ...
        n_steps = debits.shape[0]  # nb pas de temps
        # si 1 step = 1s => total_time_h = n_steps / 3600
        ...
        try:
            ...
            for gen in range(NOMBRE_GENERATIONS):
                offspring = algorithms.varAnd(
                    ...
                    mutpb=0.2   # on peut laisser TAUX_MUTATION = 0.2 ou faire un global
                )
                ...
                if generations_sans_amelioration >= 10:
                    print("Arrêt anticipé (10 gen sans amélioration).")
                    break
            ...
            print(f"Erreur executer_optimisation : {e}")
            raise
    ...
    def executer_optimisation_multi(self, callback=None):
        """
        Minimisation de (coût, énergie) + pénalités
        """
        try:
            ...
            for gen in range(NOMBRE_GENERATIONS):
                ...
                offspring = self.reproduction_genetique(
                    population, toolbox,
                    cxpb=TAUX_CROISEMENT, mutpb=0.2
                )
